...
static int atmel_flash_read(struct spi_flash *flash, u32 offset,
			    size_t len, void *buf)
{
...
static int atmel_flash_write(struct spi_flash *flash, u32 offset,
...
static int atmel_flash_erase(struct spi_flash *flash, u32 offset, size_t len)
{
...
int spi_flash_probe_atmel(struct imx_spi_flash *imx_sf, u8 *idcode)
{
	...
	for (i = 0; i < ARRAY_SIZE(imx_spi_flash_atmel_table); ++i) {
		params = &imx_spi_flash_atmel_table[i];
		if (params->idcode1 == idcode[1])
			break;
	}

	if (i == ARRAY_SIZE(imx_spi_flash_atmel_table)) {
	...
	imx_sf->flash.read  = atmel_flash_read;
	imx_sf->flash.write = atmel_flash_write;
	imx_sf->flash.erase = atmel_flash_erase;
